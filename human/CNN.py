...
class CNNBlock(nn.Module):
    def __init__(self, c_in, c_out, hdim, ks, dil, dropout):
        ...
        self.conv1 = nn.Conv1d(in_channels=c_in, out_channels=hdim, kernel_size=ks, padding='same', dilation=dil, bias = False)
        self.conv2 = nn.Conv1d(in_channels=hdim, out_channels=c_out, kernel_size=ks, padding='same', dilation=dil, bias = False)
...
class CNNLayer(nn.Module):
    def __init__(self, dim_in, dim_out, hdim, ks, dropout):
        ...
        for i in range(len(dim_out)):
            ...
            layers += [CNNBlock(current_input, current_output, hdim, ks, current_dilation, current_dropout)]
...
class Classifier(nn.Module):
    def __init__(self, dim_in, dim_out, clf_dim, layers, ks, output_size, max_len, dropout):
        ...
        self.encoder = CNNLayer(dim_in, [dim_out]*layers, dim_out*2, ks, dropout)
        self.revoClf = CNNLayer(dim_out, [clf_dim]*layers, clf_dim*2, ks, dropout)
